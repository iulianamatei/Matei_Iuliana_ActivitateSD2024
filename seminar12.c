...
int inaltime(Nod* radacina)
{
	...
	{
		int inaltimeSt=inaltime(radacina->st);//initializam inaltime de stanga, autoapelam functia
		int inaltimeDr = inaltime(radacina->dr);//la fel, dar pentru dreapta
		...
		//if-ul de mai sus scris cu operatorul ternar
...
void stergereArbore(Nod** radacina)
{
	if (*radacina)
	{
		...
		free((*radacina)->info.medii);//am sters informatia utila
		free(*radacina);
		*radacina = NULL;//aceasta modificre se face in copia din stiva
